...
% Plot data with shaded error bounds.  Removes NaN values. (by Brendan Hasz, retrieved from:
% https://github.com/brendanhasz/matlab-uncertainty-viz/blob/master/ploterr.m )
%
% USAGE
...
% Plot with X as timepoints, not grouping variable
    ...
    if strcmp(Error, 'prc') %use median + confidence intervals
        ...
        elseif strcmp(Error, 'sem') %use standard error of the mean
            ...
            errN = errP;
        elseif strcmp(Error, 'distr')
            for ll = 1:length(X)
                errP(ll) = max(Y(:,ll))-m(ll);
                errN(ll) = m(ll) - min(Y(:,ll));
            end
        else
            error(['Unimplemented Error type ' Error])
        end
