function [mySystem,TUcellNo,FcellNo,finalImage,TSC,Tm,Tm2,MutCells] = growTumor_variable(mySystem,cnst)
%Performs the agent based modeling.
%Inputs are two structures; the parameters and some global constants defined in getSystemParams (now with a chosen timestep).
...
%     ChtaxMapB = double(bwdist(Lb,'euclidean'));
    ...
    [TUcells,TUprop] = mutation_round_variable(TUcells,TUprop,TUpmut,TUantig,L,nh,TUmutmax,timestep);
...
fcount=fcount+1;
if cnst.video
    visualizeSystem_variable(TUprop,mySystem,all);
